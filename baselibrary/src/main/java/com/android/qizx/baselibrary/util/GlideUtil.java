...
public class GlideUtil {
    ...
    public static void displayAvatar(Context context,String url,ImageView view){
        Glide.with(context).load(url).bitmapTransform(new CropCircleTransformation(context)).placeholder(R.drawable.head).error(R.drawable.head).into(view);
    }
    //加载圆形头像
    ...
    //加载圆角图片
    public static void displayGlideRound(Context context,String url,ImageView view,int radian){
        Glide.with(context).load(url).transform(new GlideRoundTransform(context, radian)).placeholder(R.drawable.ic_df_avatar).into(view);
    }
    //从资源中加载
